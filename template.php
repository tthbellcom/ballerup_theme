...
function balk_preprocess_html(&$variables) {
  ...
  // Add CSS for Kultunaut on frontpage
  ...
  // Siteimprove search CSS
  drupal_add_css('http://www.siteimprove.com/searchimprove/autocomplete/autocomplete.css', array('group' => CSS_THEME, 'preprocess' => FALSE));
 ...
 */
function balk_html_head_alter(&$head_elements) {
  // Set XHTML content type in header
...
function balk_preprocess_file_entity(&$variables) {
  if ($variables['type'] == 'image') {
    ...
    if (!empty($variables['field_media_alt_text'])) {
      $variables['content']['file']['#alt'] = $variables['field_media_alt_text']['und'][0]['safe_value'];
        ...
        $referencing_entity = $variables['referencing_entity'];
        if ($referencing_entity->title) {
            $variables['content']['file']['#alt'] = $referencing_entity->title;
        }
        elseif ($referencing_entity->nid) {
            $ref_node = node_load($referencing_entity->nid);
            $variables['content']['file']['#alt'] = $ref_node->title;
        }
    }
    // Title
 ...
 */
function balk_preprocess_comment_wrapper(&$variables) {
